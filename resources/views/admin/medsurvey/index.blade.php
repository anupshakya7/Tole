				...
				<div class="card">						
					<div class="card-header border-bottom-dashed">
						<div class="row g-4 align-items-center">
							<div class="col-sm">
								<div>
									<h5 class="card-title mb-0">मेडिकल सर्वेक्षणको सूची</h5>
								</div>
							</div>
...
toastr.options = {
	...
	var medsurvey = $('.datatable-medsurvey').DataTable( {
		...
		ajax: {
				url :   "{!!route('admin.medsurvey.getmedicalsurvey')!!}",
				data: function(d){
					d.house_no =   $('#ho-house_no').val(),
					d.medical_insured =   $('#ho-medical_insured').val(),
		...
		columns:[
			...
			{data: 'medical_insured',name:'medical_insured'},
		...
		initComplete: function (settings, json) {
			this.api().columns().every(function (index) {
				...
				if(colDef.bSearchable){
					...
					}else if(colDef.data=='medical_insured'){
						var input='<select class="form-select" id="ho-'+colDef.data+'" name="'+colDef.data+'"><option value="">Select</option><option value="1">छ</option><option value="2">छैन</option></select>';
	...
	$('#ho-medical_insured').change(function () {
		medsurvey.draw();
	});
